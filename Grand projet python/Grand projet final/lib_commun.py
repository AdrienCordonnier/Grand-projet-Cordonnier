...
def ecriture_fichier_csv(contenu_fichier, nomFichier, mode):
    """
    Cette fonction a pour objectif l'ecriture dans un fichier au format csv(sep=',')
    IN : le contenu à écrire dans le fichier, le nom du fichier, le mode d'écriture
    OUT : fichier csv avec les nouvelles données
    """


    try :
        with open(f'./data/{nomFichier}', mode, newline='', encoding='utf-8') as csvfile:
